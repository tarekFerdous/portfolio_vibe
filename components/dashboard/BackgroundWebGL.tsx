...
const vertexShaderSource = `
  ...
  void main() {
    ...
    // Map fullscreen quad to NDC [-1, 1]
...
export const BackgroundWebGL = () => {
  ...
  useEffect(() => {
    ...
    // Provide full screen triangle points
    // [x,y]: (-1, -1), (3, -1), (-1, 3) 
    ...
    const uSpeedColor = gl.getUniformLocation(program, 'uSpeedColor');
    ...
    gl.uniform3fv(uColor1, hex2rgb(colors[0]));
    gl.uniform3fv(uColor2, hex2rgb(colors[1]));
    gl.uniform3fv(uColor3, hex2rgb(colors[2]));
    gl.uniform3fv(uColor4, hex2rgb(colors[3]));
    gl.uniform1f(uSpeedColor, 20.0);

    let animationFrameId: number;

    const render = (time: number) => {
      // Resize handling via exact bounds
